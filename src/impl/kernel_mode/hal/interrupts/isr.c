...
void common_isr_handler(registers_t regs) {
    // Silently ignore spurious NMIs (interrupt vector 2) so they don't halt the system.
    // NMIs can be generated by hardware/QEMU and are not useful for this OS yet.
    if (regs.int_no == 2) {
        return;
    }
